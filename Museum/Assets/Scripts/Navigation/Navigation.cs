...
public class Navigation : MonoBehaviour 
{
    ...
    public GameObject gvrEditorEmulator = null;
    private GvrEditorEmulator gvrEditorEmulatorScript = null;
	...
	void Start () 
	{
        gvrEditorEmulatorScript = gvrEditorEmulator.GetComponent<GvrEditorEmulator>();
	...
	void Update () 
	{
		if( _waypoint.Length > 0)
		{
 			...
 			for(int i = 0; i < _waypoint.Length; i++)
			{
				...
				{
                    Debug.Log("Triggered el " + _waypoint[i].name);
	...
	public void MoveTo(Waypoint waypoint)
	{
        ...
        if (distance > 0.05f)
        {
            //Debug.Log("Distancia mayor " + distance.ToString());
            //Debug.Log("Posicion camara " + view_object.transform.position.ToString());
            //Debug.Log("Punto " + waypoint.position.ToString());
            //gvrEditorEmulatorScript.enabled = false;
            view_object.transform.position = Vector3.Lerp(view_object.transform.position, waypoint.position, speed);
            //Debug.Log("Nueva posicion camara " + view_object.transform.position.ToString());
        }
        else
        {
            ...
            _current.Occupy();
            UpdateAll();
            //gvrEditorEmulatorScript.enabled = true;
            //view_object.transform.position = waypoint.position;
        }
    }
